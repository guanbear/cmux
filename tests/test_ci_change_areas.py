...
def test_macos_jobs_wait_for_linux_preflight() -> None:
    for job_name in [
        ...
        assert "      - changes" in block
        assert "      - linux-preflight" in block
        assert "if: ${{ !cancelled() && needs.changes.outputs.macos == 'true' && needs.linux-preflight.result == 'success' }}" in block
    ...
    assert "if: ${{ !cancelled() && needs.changes.outputs.macos == 'true' && needs.linux-preflight.result == 'success' && needs.swift-package-tests.result == 'success' }}" in release_block
